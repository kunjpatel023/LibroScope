...
export default function Sidebar({ collapsed, setCollapsed }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        if (res.data && res.data.profile?.user) {
  ...
  const menuItems = [
    ...
    ...(isSuperuser
      ? [{ name: "Add Book", icon: <FaPlusCircle />, path: "/add-book" }]
      : []),
  ];
  ...
  return (
    <>
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-white dark:bg-gray-900 shadow-md lg:hidden h-12">
      ...
      {/* --- Desktop Sidebar --- */}
      <div
        className={`hidden lg:flex flex-col rounded-[50px] shadow-[0_20px_50px_rgba(0,0,0,0.25)]
        h-[calc(100vh-50px)] 
      ...
      <AnimatePresence>
        {isMobileOpen && (
          ...
          >
            ...
            <div
              className="flex items-center gap-3 py-3 mt-auto border-t border-gray-300 dark:border-gray-700 cursor-pointer"
              onClick={() => {
            ...
            <Link
              to="/auth"
              onClick={() => setIsMobileOpen(false)}
              className="flex items-center gap-3 py-3 text-red-500 hover:text-red-600"
            >
              <FaSignOutAlt />
              <span>Logout</span>
            </Link>
